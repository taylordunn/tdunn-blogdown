...
* It allows for various random effect structures
...
For the `wine` data, where we have $J$ = 5 rating categories, we will build up to following mixed effects model:
...
The main reason seems to be a choice of familiar interpretation: the larger the value of any independent term $\beta x$, the smaller the thresholds $\theta_j$, and therefore a larger probability of the a response falling into a category at the upper end of the scale.
...
The $\beta_2$ coefficient estimate was statistically significant (by a Wald test), and tells us that `contact` = yes *decreases* the thresholds $\theta_j$ by $\beta_2$ = `r round(tidy(clm_rating_contact)$estimate[5], 2)`, and therefore is associated with *higher* `rating`s.

The condition number of the Hessian for this model is `r round(clm_rating_contact$cond.H, 2)`.
Larger values might indicate that the model is ill defined.

As in the primer, it is nicely illustrative to compare this model to 4 separate logistric regression with a dichomtized response:
...
The intercepts from the ordinary logistic regression correspond well to the threshold parameters $\theta_j$ from the cumulative link model.
...
For the second model, add the $\beta_1 \text{temp}_i$ term:
...
In these types of analyses, we are most often interested in the odds ratios.
For the two fixed effects, the odds ratios of $y \leq j$ comparing the two levels:
...
where we have introducted the shorthand $\gamma_j = \text{logit} (p(y \leq j))$.
...
#drop1(clm_rating_contact_temp_inter, test = "Chisq") # this accomplished the same thing
anova(clm_rating_contact_temp, clm_rating_contact_temp_inter)
```

The interaction term `contact:temp` is not support by the model.

#### Comparison to linear model

Consider the following linear model which treated `rating` as continuous:
...
See if we can spot the `judge` variance:
...
Yes, it looks like there are reasonably strong `judge`-specific effects.
`judge` 5, for isntance, doesn't stray far from `ratnig` = 3.

Fit the model with `ordinal::clmm` and logit link:
...
clmm_rating_contact_temp$ranef
...
There is no `rating` > 3 for `temp` cold and `contact` no.
Another way:
...
The inverse logit is $\text{logit}^{-1}(x) = 1 / (1 + \exp(x))$, and can be calculated with `plogis` as a shorthand.
We can subtract cumulative probabilites to get non-cumulative probabilities of a category $j$, for example:
...
which matches the value calculated previously for $j$ = 3.
...
clmm_rating_contact_temp$Theta
clmm_rating_contact_temp$beta
```


#### Choice of link function

So far, we have used the logit link (and briefly the probit link to compare to linear regression).
The ones available to `ordinal::clmm` are logit, probit, cloglog, loglog, and cauchit.

We can fit the CLMM using all of these links and compare log-likelihoods:

```{r}
#clmm_rating_contact_temp_links <-
wine %>%
  nest(everything()) %>%
  crossing(
